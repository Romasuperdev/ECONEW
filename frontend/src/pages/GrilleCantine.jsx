...
const empty = { libelle: '', montant_mois: '', montant_annee: '', nb_versements: '' }

export default function GrilleCantine() {
  ...
  const load = () => { setLoadError(''); return api.get('/cantine-tarifs').then(({ data }) => setItems(data.data || data)).catch((e) => { setItems([]); setLoadError(apiError(e)) }).finally(() => setLoading(false)) }
  useEffect(() => { load() }, [])

  const openCreate = () => { setForm(empty); setLines([]); setEditing(null); setError(''); setModal(true) }
  const openEdit = async (t) => {
    setForm({ libelle: t.libelle || '', montant_mois: t.montant_mois ?? '', montant_annee: t.montant_annee ?? '', nb_versements: '' })
    setEditing(t.id); setError(''); setModal(true)
    try { const { data } = await api.get('/echeancier', { params: { type: 'CANTINE', ref_id: t.id } }); setLines(data); setForm((f) => ({ ...f, nb_versements: String(data.length || '') })) } catch (e) { setLines([]) }
  }
  const save = async (e) => {
    e.preventDefault(); setError('')
    try {
      const payload = { libelle: form.libelle || null, montant_mois: form.montant_mois || null, montant_annee: form.montant_annee || null }
      const resp = editing ? await api.put(`/cantine-tarifs/${editing}`, payload) : await api.post('/cantine-tarifs', payload)
      const id = resp.data?.id || editing
      if (id) await api.post('/echeancier', { type: 'CANTINE', ref_id: String(id), lignes: lines })
      setModal(false); load()
    } catch (err) { setError(err.response?.data?.message || 'Erreur.') }
  }
  const remove = async (t) => { if (!confirm('Supprimer ce tarif ?')) return; try { await api.delete(`/cantine-tarifs/${t.id}`); load() } catch (err) { alert(err.response?.data?.message || 'Erreur.') } }

  return (
    <>
      <PageHeader title="Grille tarifaire — Cantine" subtitle={`${items.length} tarif(s)`} action={<Button onClick={openCreate}><Icon name="plus" size={16} /> Nouveau tarif</Button>} />
      ...
      <Card className="overflow-hidden">
        {loading ? <EmptyState message="Chargement…" /> : items.length === 0 ? <EmptyState message="Aucun tarif." /> : (
          <table className="w-full text-sm">
            <thead className="bg-brand-50 text-left text-ink"><tr><th className="px-4 py-2">Libellé</th><th className="text-right">Montant / mois</th><th className="text-right">Montant / année</th><th></th></tr></thead>
            <tbody>
              {items.map((t) => (
                <tr key={t.id} className="border-t hover:bg-brand-50">
                  <td className="px-4 py-2 font-medium">{t.libelle || '—'}</td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Modifier le tarif' : 'Nouveau tarif'} size="2xl">
        <form onSubmit={save} className="space-y-4">
          <Input label="Libellé (facultatif)" value={form.libelle} onChange={(e) => set('libelle', e.target.value)} />
